...
fig1 = df_plot(cap_df,'Gigawatts (GW)','Power Generation Capacity (Detail)')
...
fig2 = df_plot(cap_agg_df,'Gigawatts (GW)','Power Generation Capacity (Aggregate)')
...
fig3 = df_plot(gen_df,'Petajoules (PJ)','Power Generation (Detail)')
...
fig4 = df_plot(gen_agg_df,'Petajoules (PJ)','Power Generation (Aggregate)')
...
fig5 = df_plot(gen_use_df,'Petajoules (PJ)','Power Generation (Fuel use)')
...
fig6 = df_plot(dom_prd_df,'Petajoules (PJ)','Domestic energy production')
...
fig9 = df_plot(cap_cos_df,'Million $','Capital Investment')
...
# In[31]:
fig10 = ele_cos_df.iplot(asFigure=True, kind='bar',barmode='stack',x='y',title='Cost of electricity generation ($/MWh)')
